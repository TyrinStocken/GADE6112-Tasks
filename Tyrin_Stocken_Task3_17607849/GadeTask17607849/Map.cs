...
namespace GadeTask17607849
{
    class Map
    {
        ...
        public MoneyBuilding[] mB = new MoneyBuilding[1];
        ...
        List<MoneyBuilding> moneyBList = new List<MoneyBuilding>();
        ...
        public void fillMapWithMoneyBuildings(int num = 0)
        {
            for (int i = 0; i < 1; i++)
            {
                int x, y;
                x = 0;
                y = 1;
                mB[i] = new MoneyBuilding(x, y, 100, "Y", "$", 100);
                map[x, y] = mB[i].Symbol;
            }
        }
        public void loadMap()
        {
            ...
            try
            {
                inFile = new FileStream(@"Files\MoneyBuilding.txt", FileMode.Open, FileAccess.Read);
                reader = new StreamReader(inFile);
                input = reader.ReadLine();
                while (input != null)
                {
                    ...
                    faction = reader.ReadLine();
                    symbol = reader.ReadLine();
                    MoneyBuilding m = new MoneyBuilding();
                    moneyBList.Add(m);
                    input = reader.ReadLine();
                    map[xPosition, yPosition] = symbol;
                }
